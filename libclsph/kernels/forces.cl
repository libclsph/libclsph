...
float compute_density_with_grid(size_t current_particle_index, global const particle* others, const simulation_parameters params, const precomputed_kernel_values smoothing_terms, global const unsigned int* grid_cell_particle_list);
float3 compute_internal_forces_with_grid(size_t current_particle_index, global const particle* others, const simulation_parameters params, const precomputed_kernel_values smoothing_terms, global const unsigned int* grid_cell_particle_list);
    			...
    			for(size_t i = indices.x; i < indices.y; ++i) {
    				...
    				if(i != current_particle_index) {
						...
						pressure_term += 
							...
							spiky_gradient(others[current_particle_index].position - others[i].position, params.h, smoothing_terms);
					...
					normal += 
						params.particle_mass / others[i].density * 
						poly_6_gradient(others[current_particle_index].position - others[i].position, params.h, smoothing_terms);

					color_field_laplacian += 
						params.particle_mass / others[i].density * 
						poly_6_laplacian(length(others[current_particle_index].position - others[i].position), params.h, smoothing_terms);
    			}
    		}
    	}
    }
